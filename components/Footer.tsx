import { contact, nav, site, socials } from "@/data/portfolio";
...
const footerNav = nav.filter((n) =>
  ["Home", "About", "Expertise", "Projects", "Contact"].includes(n.label),
);
    ...
    <footer className="bg-[#111111] text-white pt-16 pb-10">
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-16">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              {footerNav.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="font-bold hover:text-[#ff2a2a] transition-colors"
                >
                  {item.label}
                </a>
              ))}
          ...
          <div className="md:text-right font-mono text-sm text-gray-400">
            <p>{contact.availability}</p>
            <p className="mt-1">{site.year}</p>
          </div>
        </div>

        <div className="text-center py-8 md:py-12 overflow-hidden">
          <p className="text-[15vw] md:text-[12vw] font-black leading-[0.8] tracking-tighter text-white">
            {site.brand}
          </p>
        </div>
        ...
        <div className="border-t border-white/10 pt-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          <div>
            <p className="font-medium underline underline-offset-4 decoration-1 mb-3">
              Contact
            </p>
          ...
          <div className="flex flex-col items-start md:items-center gap-3">
            <a
              ...
              className="underline underline-offset-4 decoration-1 lowercase hover:text-white transition-colors"
            >
              {contact.email}
            </a>
            <a
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-full border border-[#25D366]/20 px-4 py-1.5 text-sm text-[#25D366] hover:bg-[#25D366]/10 transition-colors"
            >
            ...
            <div className="flex md:justify-end gap-4">
              {socials.map((s) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/70 hover:text-[#ff2a2a] transition-colors"
                  aria-label={s.label}
                >
